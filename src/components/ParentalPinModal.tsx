...
interface ParentalPinModalProps {
  open: boolean;
  mode?: "verify" | "set" | "remove"; // set = create/update PIN, verify = verify existing, remove = remove after verifying
          ...
          {mode === "set" ? (
            <>
              <div>
                ...
                <Input value={pin} onChange={(e) => setPinValue(e.target.value)} type="password" />
              </div>
              <div>
                <Label>Confirme o PIN</Label>
                <Input value={pinConfirm} onChange={(e) => setPinConfirm(e.target.value)} type="password" />
              </div>
              <div className="text-sm text-muted-foreground">O PIN é armazenado localmente (hash). Ele será necessário para aprovar compras e ações sensíveis na interface.</div>
            </>
          ) : mode === "remove" ? (
            <>
              <div>
